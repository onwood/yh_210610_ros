...
#include "yh_star/yh_msg_star.h" //메시지 파일 헤더(빌드하면 자동으로 생성)

int main(int argc, char **argv) //ros::은 ros/ros.h를 불러왔기 때문에 사용 가능 - 그 뒤에 나오는 것은 ros에 종속된 함수
{
	ros::init(argc, argv, "yh_pub_star"); //노드 이름 초기화
	...
	ros::Publisher pub = nh.advertise<yh_star::yh_msg_star>("yh_topic_star", 100); 

	ros::Rate loop_rate(2); //루프 주기를 10Hz로 설정 - 1초 당 10씩, loop_rate라는 이름은 개인이 만든 것
	
	yh_star::yh_msg_star msg; //개인이 만든 메시지 파일 형식으로 (  )라는 메시지를 선언
	int count = 0;
